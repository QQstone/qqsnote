...
与数组相比 链表在内存中不是连续的 通过内部的指针(js中是对象的引用)串联在一起

至少两部分：数据域&指针域 链表没有索引 查找复杂度O(n)

+ 访问 必须从头节点逐节点获取引用
+ 增删 只需要改动节点引用(而数组增删会改动所有后续受影响的项)

> React Fiber底层使用链表结构 以支持渲染的暂停、中止或复用渲染任务，实现了可中断的增量渲染，极大地提升了复杂应用的性能。

与链表有关的算法题 常用哨兵节点(dummy node)和快慢指针

dummy.next = head 避免丢失head
...
var reverseList = function(head) {
    ...
    // 当前节点follow子链表
    // 此时的子链表的最后一个节点(地址) 仍然是之前head.next节点(地址)
    // 该地址next反转指向head
    head.next.next = head
    head.next = null
    return newHead
...
+ map.set(key, value)
+ map.get(key)
+ map.clear() map.delete(x)
+ map.keys() map.values() map.entries()
+ map 与 二维数组 相互转换 map=new Map([['key1','value1'],['key2','value2']])

用于快速查找， 两数之和问题中 循环过的元素+下标放入Map 循环新元素时其与target的差值 如果在Map.has True 则可get到下标

Set

基于哈希表实现的去重数组 相当于map(value,value)

+ set.has(x)
+ set.add(x)
+ set.clear() set.delete(x)
+ set.keys() set.values() set.entries()三者输出相同
+ set = new Set(array); array = Array.from(set)
