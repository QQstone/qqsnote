...
TCproto.GetTags = function() {
  var el = this.SourceElements(), etl, tl = [], i, j, k, dataWeights;
  for(k = 0; k <= this.repeatTags; ++k) {
    for(i = 0; i < el.length; ++i) {
      etl = el[i].getElementsByTagName('a');
      // tag count
      var tag_count_list = el[i].getElementsByTagName('span');
      if(tag_count_list&&tag_count_list.length===etl.length){
        dataWeights = Array.prototype.slice.call(tag_count_list).map(element=>(element.textContent))
      }
      for(j = 0; j < etl.length; ++j) {
        if(dataWeights){
          etl[j].setAttribute('data-weight', dataWeights[j])
        }
        tl.push(etl[j]);
      }
    }
